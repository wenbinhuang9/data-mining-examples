...
carnn <- neuralnet(Price_r ~ Age_08_04 + KM + HP + Met_Color + CC + Doors + Quarterly_Tax + Weight + Automatic, 
                   data = cartrain.df, linear.output = F, hidden = 4)
# display weights
carnn$weights
# plot network
plot(carnn, rep="best")
...
#put into daraframe
pricepred_unlog.df<-data.frame(predict = pricepred_unlog)
head(pricepred_unlog.df)
...
library(Metrics)
print(rmse(carvalid.df$Price, pricepred_unlog.df$predict))

library(forecast)
accuracy(pricepred_unlog.df$predict,carvalid.df$Price)

print(pricepred_unlog.df$predict)
print(carvalid.df$Price)

dim(pricepred_unlog.df)
dim(carvalid.df)
